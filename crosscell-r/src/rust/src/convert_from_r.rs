...
pub fn r_to_expression(obj: Robj) -> Result<ExpressionMatrix> {
    if obj.inherits("dgCMatrix") {
        dgcmatrix_to_expression(obj)
    } else if obj.is_matrix() {
        rmatrix_to_expression(obj)
    } else {
        Err("Expected dgCMatrix or matrix".into())
    }
}
...
fn dgcmatrix_to_expression(obj: Robj) -> Result<ExpressionMatrix> {
    ...
    let csc = SparseMatrixCSC::new(x_slot, indices, indptr, n_rows, n_cols)
...
fn rmatrix_to_expression(obj: Robj) -> Result<ExpressionMatrix> {
    let data: Vec<f64> = obj
        .as_real_vector()
        .ok_or("Cannot convert matrix to real vector")?;
    let dim: Vec<i32> = obj
        .dim()
        .ok_or("Matrix has no dimensions")?
        .iter()
        .map(|x| x.inner())
        .collect();
    let n_rows = dim[0] as usize;
    ...
    for row in 0..n_rows {
        for col in 0..n_cols {
            row_major[row * n_cols + col] = data[col * n_rows + row];
        }
    }
...
fn r_vector_to_arrow(obj: &Robj) -> Result<ArrayRef> {
    if obj.inherits("factor") {
        return r_factor_to_arrow(obj);
    }
    match obj.rtype() {
        Rtype::Integers => {
            let values: Vec<i32> = obj.as_integer_vector().unwrap();
            let av: Vec<Option<i64>> = values
                .iter()
                .map(|&v| if v == i32::MIN { None } else { Some(v as i64) })
                .collect();
            Ok(Arc::new(Int64Array::from(av)))
        }
        Rtype::Doubles => {
            let values: Vec<f64> = obj.as_real_vector().unwrap();
            let av: Vec<Option<f64>> = values
                .iter()
        ...
        Rtype::Logicals => {
            ...
            let av: Vec<Option<bool>> = values
                .iter()
                .map(|&v| if v.is_na() { None } else { Some(v.is_true()) })
                .collect();
        ...
        _ => {
            let obj_clone = obj.clone();
            let sv: Vec<String> = R!("as.character({{obj_clone}})")?
                .as_string_vector()
                .unwrap_or_default();
...
pub fn r_to_embedding(name: &str, obj: Robj) -> Result<Embedding> {
    ...
    let data: Vec<f64> = mat
        .as_real_vector()
        .ok_or("Cannot convert to real vector")?;
    let dim: Vec<i32> = mat
        .dim()
        .ok_or("Matrix has no dimensions")?
        .iter()
    ...
    for row in 0..n_rows {
        for col in 0..n_cols {
            rm[row * n_cols + col] = data[col * n_rows + row];
        }
    }
...
pub fn seurat_to_ir(obj: Robj) -> Result<SingleCellData> {
    ...
    let o1 = obj.clone();
    let aa: String = R!("Seurat::DefaultAssay({{o1}})")?
        .as_str()
        .unwrap_or("RNA")
        .to_string();
    let aa1 = aa.clone();
    ...
    let gene_metadata = if gm.is_null() {
        DataFrame::empty(n_genes)
    } else {
        r_to_dataframe(gm).unwrap_or_else(|_| DataFrame::empty(n_genes))
    };
    ...
    let mut data = SingleCellData::new(expression, cell_metadata, gene_metadata, metadata)
        .map_err(|e| Error::Other(e))?;
    let o7 = obj.clone();
    let rn: Vec<String> = R!("names({{o7}}@reductions)")?
        .as_string_vector()
        .unwrap_or_default();
    if !rn.is_empty() {
        ...
        for name in &rn {
            ...
            if let Ok(emb) = r_to_embedding(name, dr) {
                embs.insert(name.clone(), emb);
            }
        }
        if !embs.is_empty() {
            data.embeddings = Some(embs);
        }
    }
    let aa3 = aa.clone();
    let o9 = obj.clone();
    ...
    if !dm.is_null() && dm.len() > 0 {
        ...
        if let Ok(expr) = r_to_expression(dm) {
            layers.insert("data".to_string(), expr);
        }
        if !layers.is_empty() {
...
pub fn sce_to_ir(obj: Robj) -> Result<SingleCellData> {
    if !obj.inherits("SingleCellExperiment") {
        return Err("Expected SingleCellExperiment object".into());
    }
    let o1 = obj.clone();
    ...
    let mut data = SingleCellData::new(expression, cell_metadata, gene_metadata, metadata)
        .map_err(|e| Error::Other(e))?;
    let o6 = obj.clone();
    let rdn: Vec<String> = R!("SingleCellExperiment::reducedDimNames({{o6}})")?
        .as_string_vector()
        .unwrap_or_default();
    if !rdn.is_empty() {
        let mut embs = HashMap::new();
        for name in &rdn {
            ...
            if let Ok(emb) = r_to_embedding(name, mat) {
                embs.insert(name.clone(), emb);
            }
        }
        if !embs.is_empty() {
    ...
    let o8 = obj.clone();
    let an: Vec<String> = R!("SummarizedExperiment::assayNames({{o8}})")?
        .as_string_vector()
        .unwrap_or_default();
    ...
    if !oa.is_empty() {
        ...
        for name in oa {
            ...
            if let Ok(expr) = r_to_expression(assay) {
                layers.insert(name.clone(), expr);
            }
        }
        if !layers.is_empty() {
            data.layers = Some(layers);
...
pub fn r_to_unstructured(obj: Robj) -> Result<UnstructuredValue> {
    ...
    if obj.is_list() {
        ...
        for i in 1..=len {
            let o3 = obj.clone();
            let item = R!("{{o3}}[[{{i}}]]")?;
            arr.push(r_to_unstructured(item)?);
        }
    ...
    match obj.rtype() {
        Rtype::Logicals => {
            ...
            if v.len() == 1 {
                if v[0].is_na() {
                    Ok(UnstructuredValue::Null)
                } else {
                    Ok(UnstructuredValue::Boolean(v[0].is_true()))
                }
            } else {
                Ok(UnstructuredValue::Array(
                    v.iter()
                        .map(|&x| {
                            if x.is_na() {
                                UnstructuredValue::Null
                            } else {
                                UnstructuredValue::Boolean(x.is_true())
                            }
                        })
                        .collect(),
                ))
        ...
        Rtype::Integers => {
            ...
            if v.len() == 1 {
                if v[0] == i32::MIN {
                    ...
                    Ok(UnstructuredValue::Integer(v[0] as i64))
                }
            } else {
                Ok(UnstructuredValue::Array(
                    v.iter()
                        .map(|&x| {
                            if x == i32::MIN {
                                ...
                                UnstructuredValue::Integer(x as i64)
                            }
                        })
                        .collect(),
                ))
        ...
        Rtype::Doubles => {
            ...
            if v.len() == 1 {
                if v[0].is_nan() {
                    ...
                    Ok(UnstructuredValue::Float(v[0]))
                }
            } else {
                Ok(UnstructuredValue::Array(
                    v.iter()
        ...
        Rtype::Strings => {
            ...
            if v.len() == 1 {
                Ok(UnstructuredValue::String(v[0].clone()))
            } else {
                Ok(UnstructuredValue::Array(
                    v.iter()
                        .map(|s| UnstructuredValue::String(s.clone()))
                        .collect(),
                ))
            }
        }
        _ => {
            let o = obj.clone();
            let s: String = R!("as.character({{o}})[[1]]")?
                .as_str()
                .unwrap_or("")
                .to_string();
