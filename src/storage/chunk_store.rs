...
use crate::ir::expression::{SparseFormat, SparseMatrixCSR, SparseMatrixCSC};
...
impl ChunkStore {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        if version != VERSION {
            return Err(CrossCellError::InvalidFormat(
                format!("Unsupported chunk store version: {}", version)
            ));
        }
        ...
        let sparse_format = match format_byte[0] {
            0 => SparseFormat::CSR,
            1 => SparseFormat::CSC,
            _ => return Err(CrossCellError::InvalidFormat(
    ...
    pub fn read_csr_chunk(&self, chunk_idx: usize) -> Result<SparseMatrixCSR> {
        ...
        let decompressed = if self.compression {
            lz4_flex::decompress_size_prepended(&data)
                .map_err(|e| CrossCellError::InvalidFormat(format!("Decompression failed: {}", e)))?
        } else {
            data
        };
