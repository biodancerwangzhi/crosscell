...
use crate::ir::expression::{
    ExpressionMatrix, SparseFormat, SparseMatrixCSR, SparseMatrixCSC,
};
...
impl DiskBackedMatrix {
        ...
        F: FnMut(usize, ExpressionMatrix) -> Result<T>,
    {
        ...
        for chunk_idx in 0..num_chunks {
            let chunk = match self.sparse_format {
                SparseFormat::CSR => {
                    ExpressionMatrix::SparseCSR(self.read_csr_chunk(chunk_idx)?)
                }
                SparseFormat::CSC => {
                    ExpressionMatrix::SparseCSC(self.read_csc_chunk(chunk_idx)?)
                }
            };
    ...
    fn merge_csr_chunks(&self) -> Result<ExpressionMatrix> {
        ...
        let matrix = SparseMatrixCSR::new(
            data, indices, indptr, self.total_rows, self.total_cols
