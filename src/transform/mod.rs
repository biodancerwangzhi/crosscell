...
use crate::ir::{
    DenseMatrix, ExpressionMatrix, SingleCellData, SparseMatrixCSC, SparseMatrixCSR,
};
use log::warn;
...
/// 全零行保持为零（避免除零）
pub fn normalize_library_size(
    matrix: &ExpressionMatrix,
) -> Result<ExpressionMatrix, String> {
    match matrix {
        ExpressionMatrix::SparseCSR(csr) => {
            normalize_csr(csr).map(ExpressionMatrix::SparseCSR)
        }
        ExpressionMatrix::SparseCSC(csc) => {
            normalize_csc(csc).map(ExpressionMatrix::SparseCSC)
        }
        ExpressionMatrix::Dense(dense) => {
            normalize_dense(dense).map(ExpressionMatrix::Dense)
        }
        ExpressionMatrix::Lazy(_) => {
            Err("Cannot normalize lazy matrix; load it first".to_string())
        }
    }
}
...
/// N > 基因数时输出警告并保留所有基因。
pub fn select_top_variable_genes(
    data: &mut SingleCellData,
    n_genes: usize,
) -> Result<(), String> {
    ...
    indices.sort_by(|&a, &b| variances[b].partial_cmp(&variances[a]).unwrap_or(std::cmp::Ordering::Equal));
...
fn filter_matrix_columns(
    ...
    match matrix {
        ...
        ExpressionMatrix::Lazy(_) => {
            Err("Cannot filter columns on lazy matrix".to_string())
        }
    }
}
...
fn filter_dataframe_rows(
    ...
    let indices_array = UInt32Array::from(
        keep_indices.iter().map(|&i| i as u32).collect::<Vec<u32>>(),
    );
...
pub fn apply_gene_id_column(
    ...
    let string_values: Vec<String> = if let Some(arr) = col_data.as_any().downcast_ref::<StringArray>() {
        (0..arr.len())
            .map(|i| {
                if arr.is_null(i) {
                    String::new()
        ...
        (0..arr.len())
            .map(|i| {
                if arr.is_null(i) {
                    ...
                    arrow::util::display::array_value_to_string(arr, i)
                        .unwrap_or_default()
                }
            })
            .collect()
    ...
    let new_index_array: ArrayRef =
        std::sync::Arc::new(StringArray::from(string_values));
