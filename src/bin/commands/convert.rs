...
use crosscell::anndata::{
    streaming_convert, streaming_h5ad_to_rds, streaming_rds_to_h5ad,
    StreamingH5adReader,
};
...
use super::progress::{StageProgress, format_bytes};
...
pub fn run(
    ...
    let input_converter = registry.detect(&input)
    ...
    let output_converter = registry.get(&target_format)
        .context(format!("Unknown target format: {}. Use 'crosscell formats' to list supported formats.", format))?;
    ...
    let should_use_streaming = streaming || should_auto_enable_streaming(&input, file_size, &input_format);
    ...
    println!("   Format: {} → {}", input_format.to_uppercase(), target_format.to_uppercase());
    ...
    if disk_backed {
        let temp = temp_dir.as_ref().map(|p| p.display().to_string())
    ...
    let mut ir_data = if use_direct_read {
        ...
        match read_seurat_direct(&input, keep_layers) {
            Ok(result) => {
                ...
                let detail = format!("Seurat {} - {} cells × {} genes", 
                ...
                if result.skipped.has_skipped() {
                    ...
                    if !skipped.closures.is_empty() {
                        progress.verbose(&format!("⚠ Skipped {} closure(s)", skipped.closures.len()));
                    }
                    if !skipped.environments.is_empty() {
                        progress.verbose(&format!("⚠ Skipped {} environment(s)", skipped.environments.len()));
                    }
                    if !skipped.external_pointers.is_empty() {
                        progress.verbose(&format!("⚠ Skipped {} external pointer(s)", skipped.external_pointers.len()));
                    }
                    if !skipped.bytecodes.is_empty() {
                        progress.verbose(&format!("⚠ Skipped {} bytecode(s)", skipped.bytecodes.len()));
                    }
                }
            ...
            Err(direct_err) => {
                ...
                let data = input_converter.read(&input)
                    .map_err(|e| anyhow::anyhow!("Failed to read {} file: {}", input_format, e))?;
                
                let detail = format!("{} cells × {} genes (simplified)", 
        ...
        progress.update_message(&format!("parsing {} structure", input_format.to_uppercase()));
        
        let data = input_converter.read(&input)
            .map_err(|e| anyhow::anyhow!("Failed to read {} file: {}", input_format, e))?;
        
        let detail = format!("{} cells × {} genes", 
            data.metadata.n_cells, data.metadata.n_genes);
        progress.complete_stage(Some(&detail));
    ...
    if dry_run {
        progress.finish();
        print_dry_run_summary(&input, &output, &input_format, &target_format, 
            &ir_data, &diagnostic_report, &cleaning_preview)?;
        return Ok(());
    }
    ...
    if diagnostic_report.auto_fixable_count > 0 {
        ...
        if cleaning_report.has_modifications() {
            progress.verbose(&format!("Fixed {} issue(s)", cleaning_report.total_modifications));
    ...
    progress.next_stage();
    progress.update_message(&format!("preparing {} output", target_format.to_uppercase()));
    ...
    output_converter.write(&ir_data, &output)
    ...
    if validate {
        ...
        let converted = output_converter.read(&output)
            .map_err(|e| anyhow::anyhow!("Failed to read converted file for validation: {}", e))?;
        
        let validation_result = validation::roundtrip::validate_roundtrip(&ir_data, &converted, 1e-7);
...
fn print_memory_estimate(input: &PathBuf, lazy: bool, disk_backed: bool, chunk_size: usize) -> Result<()> {
    let file_size = std::fs::metadata(input)?.len();
    let estimated_full = file_size as f64 * 3.0;
    let estimated_lazy = file_size as f64 * 0.5;
    let estimated_disk_backed = (chunk_size as f64 * 30000.0 * 8.0 * 2.0) / (1024.0 * 1024.0 * 1024.0); // ~2 chunks in memory
    ...
    if disk_backed {
        ...
        println!("   Chunk size: {} rows", chunk_size);
        println!("   Disk usage: ~{}", format_bytes((file_size as f64 * 0.7) as usize));
    } else if lazy {
        println!("   Lazy load: ~{} (peak)", format_bytes(estimated_lazy as usize));
        println!("   Chunk size: {} rows", chunk_size);
    }
...
fn print_dry_run_summary(
    ...
    println!("─────────────────────────────────────────");
    println!();
    println!("Input:  {} ({})", input.display(), input_format.to_uppercase());
    println!("Output: {} ({})", output.display(), target_format.to_uppercase());
    ...
    if cleaning_preview.has_modifications() {
        println!("🔧 Planned modifications: {}", cleaning_preview.total_modifications);
        
        if !cleaning_preview.columns_renamed.is_empty() {
            println!("   Rename {} column(s)", cleaning_preview.columns_renamed.len());
        }
        if !cleaning_preview.duplicates_resolved.is_empty() {
            println!("   Deduplicate {} name(s)", cleaning_preview.duplicates_resolved.len());
        }
        if !cleaning_preview.names_truncated.is_empty() {
            println!("   Truncate {} name(s)", cleaning_preview.names_truncated.len());
        }
        if !cleaning_preview.columns_dropped.is_empty() {
            println!("   Drop {} column(s)", cleaning_preview.columns_dropped.len());
        }
        println!();
    } else {
    ...
    println!("   Output size: ~{}", format_bytes(estimated_output as usize));
    println!("   Compatibility: {}/100", diagnostic_report.compatibility_score);
    println!();
    
    println!("💡 To proceed with conversion, run without --dry-run:");
    println!("   crosscell convert -i {} -o {} -f {}", 
        input.display(), output.display(), target_format);
...
fn run_streaming_conversion(
    ...
    if dry_run {
        ...
        println!("   crosscell convert -i {} -o {} -f {} --streaming --chunk-size {}",
            input.display(), output.display(), target_format, chunk_size);
        return Ok(());
    }
    ...
    if let Ok(output_meta) = std::fs::metadata(&output) {
        println!("   Output size: {}", format_bytes(output_meta.len() as usize));
    }
...
fn run_h5ad_to_h5ad_streaming(input: &PathBuf, output: &PathBuf, chunk_size: usize) -> Result<()> {
    ...
    let progress_callback = |current: usize, total: usize| {
        if current > processed {
            ...
            print!("\r   Progress: {}% ({}/{} cells)", pct, format_number(current), format_number(total));
