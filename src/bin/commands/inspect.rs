...
pub fn run(input: PathBuf, detailed: bool, output: Option<PathBuf>) -> Result<()> {
    ...
    let data = match format.as_str() {
        "AnnData (.h5ad)" => {
            ...
            anndata::reader::read_h5ad(input_str).context("Failed to read H5AD file")?
        }
        "Seurat (.rds)" => {
            let input_str = input.to_str().context("Invalid input path")?;
    ...
    println!(
        "  ├─ Expression matrix: {}",
        if sparsity > 50.0 { "Sparse" } else { "Dense" }
    );
    println!(
        "  ├─ Cell metadata (obs): {} columns",
        data.cell_metadata.n_cols()
    );
    println!(
        "  ├─ Gene metadata (var): {} columns",
        data.gene_metadata.n_cols()
    );

    if has_embeddings {
        let emb_count = data.embeddings.as_ref().map_or(0, |e| e.len());
        let emb_names: Vec<_> = data
            .embeddings
            .as_ref()
    ...
    if diagnostic_report.has_issues() {
        ...
        println!(
            "⚠️  Detected {} issue(s){}:",
            diagnostic_report.total_issues, auto_fix_text
        );
        ...
        for issue in &diagnostic_report.issues {
            ...
            let fix_hint = if issue.auto_fixable {
                " → will auto-fix"
            } else {
                ""
            };
    ...
    if let Some(output_path) = output {
        ...
        let issues_json: Vec<_> = diagnostic_report
            .issues
            .iter()
            .map(|issue| {
                json!({
