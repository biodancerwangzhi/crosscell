...
use crate::ir::{
    ...
    SingleCellData, SparseMatrixCSR,
};
...
fn read_loom(path: &Path) -> Result<SingleCellData, CrossCellError> {
    let file = H5File::open(path)
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to open loom file: {}", e)))?;

    // 读取主矩阵 (genes × cells)
    let matrix_ds = file
        .dataset("matrix")
        .map_err(|e| CrossCellError::MissingField {
            field: format!("/matrix: {}", e),
        })?;
    ...
    let matrix_data: Vec<f64> = matrix_ds
        .read_raw()
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to read matrix data: {}", e)))?;
...
fn read_col_attrs(
    file: &H5File,
    n_cells: usize,
) -> Result<(DataFrame, Option<HashMap<String, Embedding>>), CrossCellError> {
    ...
    let col_attrs = file
        .group("col_attrs")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to open /col_attrs: {}", e)))?;
    ...
    let cell_metadata = if columns.is_empty() {
        ...
        DataFrame::new(columns, data, n_cells).map_err(|e| {
            CrossCellError::InvalidFormat(format!(
                "Failed to create cell metadata DataFrame: {}",
                e
            ))
        })?
    };
...
/// 读取属性数据集
fn read_attribute_dataset(
    ds: &hdf5::Dataset,
    expected_len: usize,
) -> Result<Option<ArrayRef>, CrossCellError> {
    let shape = ds.shape();
...
fn read_row_attrs(file: &H5File, n_genes: usize) -> Result<DataFrame, CrossCellError> {
    ...
    let row_attrs = file
        .group("row_attrs")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to open /row_attrs: {}", e)))?;
    ...
    if columns.is_empty() {
        ...
        DataFrame::new(columns, data, n_genes).map_err(|e| {
            CrossCellError::InvalidFormat(format!(
                "Failed to create gene metadata DataFrame: {}",
                e
            ))
        })
    }
...
fn read_layers(
    ...
    n_cells: usize,
    n_genes: usize,
) -> Result<Option<HashMap<String, ExpressionMatrix>>, CrossCellError> {
    if !file.link_exists("layers") {
        return Ok(None);
    }

    let layers_group = file
        .group("layers")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to open /layers: {}", e)))?;
    ...
    for name in member_names {
        if let Ok(ds) = layers_group.dataset(&name) {
            ...
            if let Ok(matrix_data) = ds.read_raw::<f64>() {
                ...
                layers.insert(
                    name,
                    ExpressionMatrix::Dense(DenseMatrix {
                        n_rows: n_cells,
                        n_cols: n_genes,
                        data: transposed,
                    }),
...
fn read_graphs(
    ...
    (
        Option<HashMap<String, PairwiseMatrix>>,
        Option<HashMap<String, PairwiseMatrix>>,
    ),
    CrossCellError,
> {
...
fn read_graph_data(group: &Group, n: usize) -> Result<Option<PairwiseMatrix>, CrossCellError> {
    ...
    let a_ds = group
        .dataset("a")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to open graph/a: {}", e)))?;
    ...
    let b_ds = group
        .dataset("b")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to open graph/b: {}", e)))?;
...
fn write_loom(data: &SingleCellData, path: &Path) -> Result<(), CrossCellError> {
    let file = H5File::create(path)
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to create loom file: {}", e)))?;
...
fn write_layers_group(
    ...
    let layers_group = file
        .create_group("layers")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to create /layers: {}", e)))?;
...
fn write_graph_to_group(
    ...
    let a_ds = graph_group
        ...
        .shape([a.len()])
        .create("a")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to create graph/a: {}", e)))?;
    a_ds.write(&a)
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to write graph/a: {}", e)))?;
    ...
    let b_ds = graph_group
        ...
        .shape([b.len()])
        .create("b")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to create graph/b: {}", e)))?;
    b_ds.write(&b)
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to write graph/b: {}", e)))?;
    ...
    let w_ds = graph_group
        ...
        .shape([w.len()])
        .create("w")
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to create graph/w: {}", e)))?;
    w_ds.write(&w)
        .map_err(|e| CrossCellError::InvalidFormat(format!("Failed to write graph/w: {}", e)))?;
