...
impl FormatConverter for SceConverter {
    ...
    fn read(&self, path: &Path) -> Result<SingleCellData, CrossCellError> {
        let path_str = path.to_str().ok_or_else(|| {
            CrossCellError::InvalidFormat("Invalid path encoding".to_string())
        })?;
        
        sce_rds_to_ir(path_str).map_err(|e| {
            CrossCellError::InvalidFormat(format!("Failed to read SCE object: {}", e))
        })
    }
