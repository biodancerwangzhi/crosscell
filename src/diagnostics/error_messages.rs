...
fn get_suggestion(error: &CrossCellError) -> Option<String> {
    match error {
        CrossCellError::FileNotFound { path } => Some(format!(
                ...
                Some(
                    "Solution: Ensure the file is a valid Seurat RDS file.\n\
                     ...
                     If the file is corrupted, try re-saving from R:\n\
                       ...
                       saveRDS(seurat_obj, 'file_fixed.rds')"
                        .to_string(),
                )
            } else {
                Some(
        ...
        CrossCellError::UnsupportedFormat { expected, actual } => Some(format!(
        ...
        CrossCellError::DimensionMismatch { context } => Some(format!(
        ...
        CrossCellError::OutOfMemory { context } => Some(format!(
        ...
        CrossCellError::ConversionFailed { from, to, reason } => Some(format!(
        ...
        CrossCellError::UnsupportedType { type_name } => Some(format!(
        ...
        CrossCellError::InvalidSparseMatrix(msg) => Some(format!(
        ...
        CrossCellError::ValidationFailed(msg) => Some(format!(
...
mod tests {
    ...
    fn test_error_report() {
        let error = CrossCellError::file_not_found("test.h5ad");
        let context = ErrorContext::new("conversion").with_input("test.h5ad");
