...
use crate::sce::extract::{
    ensure_sce_class, extract_slot, extract_slot_optional, get_named_list_items,
    extract_integer_vector, extract_real_vector,
};
...
fn parse_sce(robj: &RObject, file: &RdsFile) -> Result<SingleCellData> {
    ...
    log::debug!("Extracted expression matrix: {} cells × {} genes", n_cells, n_genes);
    ...
    log::debug!("Extracted cell metadata: {} columns", cell_metadata.n_cols());
    ...
    log::debug!("Extracted gene metadata: {} columns", gene_metadata.n_cols());
...
fn parse_assays(assays_slot: &RObject, _file: &RdsFile) -> Result<(ExpressionMatrix, Option<HashMap<String, ExpressionMatrix>>)> {
    ...
    let assay_list = get_named_list_items(assay_data)
        .ok_or_else(|| SceError::InvalidAssaysStructure)?;
    ...
    let (i, p, x, dim) = match robj {
        RObject::S4Object(s4) => {
            let i = s4.attributes.get("i")
                .ok_or_else(|| SceError::ParseError("Missing 'i' slot in dgCMatrix".to_string()))?;
            let p = s4.attributes.get("p")
                .ok_or_else(|| SceError::ParseError("Missing 'p' slot in dgCMatrix".to_string()))?;
            let x = s4.attributes.get("x")
                .ok_or_else(|| SceError::ParseError("Missing 'x' slot in dgCMatrix".to_string()))?;
            let dim = s4.attributes.get("Dim")
                .ok_or_else(|| SceError::ParseError("Missing 'Dim' slot in dgCMatrix".to_string()))?;
            (i, p, x, dim)
        }
        _ => {
...
fn parse_reduced_dims(int_col_data: &RObject, expected_rows: usize, file: &RdsFile) -> Result<Option<HashMap<String, Embedding>>> {
...
fn parse_reduced_dims_list(reduced_dims: &RObject, expected_rows: usize, _file: &RdsFile) -> Result<Option<HashMap<String, Embedding>>> {
...
fn robject_to_arrow_array(
    ...
    match robj {
        ...
        _ => {
            ...
            log::warn!("Unsupported RObject type for column {}: {:?}, using placeholder", col_name, robj.type_name());
