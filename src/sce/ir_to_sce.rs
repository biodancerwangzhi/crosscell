...
use crate::ir::{DataFrame as IrDataFrame, Embedding, ExpressionMatrix, SingleCellData};
use crate::rds::StringEncoding;
use crate::rds::{
    write_rds, Attributes, DoubleVector, GenericVector, IntegerVector, RObject, RdsFile, S4Object,
    StringVector,
};
...
fn arrow_array_to_robject(array: &arrow::array::ArrayRef) -> Result<RObject> {
    ...
    match array.data_type() {
        ...
        _ => Err(SceError::ConversionError(format!(
...
fn arrow_dict_to_factor(array: &arrow::array::ArrayRef) -> Result<RObject> {
    ...
    let levels: Vec<String> = if let Some(str_arr) = values.as_any().downcast_ref::<StringArray>() {
        ...
        return Err(SceError::ConversionError(
            "Dictionary values must be strings".to_string(),
        ));
    };
...
fn build_matrix(emb: &Embedding) -> Result<RObject> {
    ...
    set_attr(
        ...
        "dim",
        RObject::IntegerVector(IntegerVector {
            data: vec![n_rows as i32, n_cols as i32],
            attributes: Attributes::new(),
        }),
    );
...
fn build_sce_object(
    ...
    set_attr(
        ...
        "nrows",
        RObject::IntegerVector(IntegerVector {
            data: vec![ir.metadata.n_genes as i32],
            attributes: Attributes::new(),
        }),
    );
