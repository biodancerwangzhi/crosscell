...
pub fn merge_csr_row_wise(
    matrices: &[&SparseMatrixCSR],
) -> Result<SparseMatrixCSR, SeuratError> {
...
pub fn merge_split_layers(
    ...
    let mut prefix_groups: HashMap<String, Vec<(String, String, ExpressionMatrix)>> = HashMap::new();
    ...
    for (prefix, mut members) in prefix_groups {
        if members.len() == 1 {
            ...
            log::info!(
                ...
                members.len(),
                prefix,
                members.iter().map(|(_, s, _)| s.as_str()).collect::<Vec<_>>()
            );
...
pub fn pad_split_layers_to_full_size(
    ...
    for name in raw_layers.keys() {
        if let Some(caps) = re.captures(name) {
            ...
            prefix_groups.entry(prefix).or_default().push((name.clone(), suffix));
        }
    }
    ...
    for (prefix, mut members) in prefix_groups {
        ...
        for (layer_name, _suffix) in &members {
            ...
            log::info!(
                ...
                layer_name, sub_cells, cell_offset, total_cells
            );
        ...
        if cell_offset != total_cells {
            log::warn!(
                ...
                prefix, cell_offset, total_cells
            );
        }
    }
...
fn pad_matrix_cells(
    ...
    match matrix {
        ...
        ExpressionMatrix::SparseCSC(csc) => {
            ...
            let new_indices: Vec<usize> = csc.indices.iter()
                .map(|&idx| idx + cell_offset)
                .collect();
