...
use crate::ir::{
    DataFrame, DatasetMetadata, Embedding, ExpressionMatrix, SingleCellData, SpatialData,
};
...
impl SkippedComponents {
    ...
    pub fn has_skipped(&self) -> bool {
        !self.environments.is_empty()
            ...
            || !self.external_pointers.is_empty()
...
pub fn read_seurat_direct<P: AsRef<Path>>(
    path: P,
    keep_layers: bool,
) -> Result<DirectReadResult, SeuratError> {
    ...
    let data = match version {
        SeuratVersion::V3 | SeuratVersion::V4 => {
            extract_seurat_v3_v4(&file.object, &file, &mut skipped)?
        }
        SeuratVersion::V5 | SeuratVersion::Unknown => {
            extract_seurat_v5(&file.object, &file, &mut skipped, keep_layers)?
        }
    };
    Ok(DirectReadResult {
        data,
    ...
    match obj {
        RObject::GenericVector(gv) => {
            let names: Vec<&str> = gv.attributes.get("names").and_then(|n| {
                if let RObject::StringVector(sv) = n {
                    Some(sv.data.iter().map(|s| s.as_str()).collect())
                } else {
                    None
                }
            })?;
...
fn should_skip_object(robj: &RObject, name: &str, skipped: &mut SkippedComponents) -> bool {
    match robj {
        RObject::EnvironmentIndex { .. } => {
            skipped.environments.push(name.to_string());
            true
        }
        RObject::ExternalPointerIndex(_) => {
            skipped.external_pointers.push(name.to_string());
            true
        }
        RObject::LanguageObject(_) => {
            skipped.languages.push(name.to_string());
            true
        }
        RObject::BuiltInFunction(_) => {
            skipped.closures.push(name.to_string());
            true
        }
        _ => false,
    }
        ...
        if let Some(items) = get_named_list_items(assays) {
            if let Some((_, first_assay)) = items.first() {
                return Ok(match detect_assay_type(first_assay) {
                    ...
                    AssayType::Legacy => {
                        if s4.attributes.get("images").is_some() {
                            SeuratVersion::V4
                        } else {
                            SeuratVersion::V3
                        }
...
fn detect_assay_type(robj: &RObject) -> AssayType {
    ...
    if let RObject::S4Object(s4) = robj {
        if s4.class_name.contains("Assay5") {
            return AssayType::Assay5;
        }
        if s4.class_name.contains("Assay") {
...
fn parse_dgcmatrix(robj: &RObject) -> Result<ExpressionMatrix, SeuratError> {
    ...
    let i = s4
        .attributes
        .get("i")
        .ok_or_else(|| SeuratError::ParseError("Missing 'i' in dgCMatrix".into()))?;
    let p = s4
        .attributes
        .get("p")
        .ok_or_else(|| SeuratError::ParseError("Missing 'p' in dgCMatrix".into()))?;
    let x = s4
        .attributes
        .get("x")
        .ok_or_else(|| SeuratError::ParseError("Missing 'x' in dgCMatrix".into()))?;
    let dim = s4
        .attributes
        .get("Dim")
        .ok_or_else(|| SeuratError::ParseError("Missing 'Dim' in dgCMatrix".into()))?;
...
fn extract_assay5_layers(robj: &RObject) -> Result<HashMap<String, ExpressionMatrix>, SeuratError> {
    ...
    let layers_obj = if let RObject::S4Object(s4) = robj {
        ...
        let fields = get_named_list_items(robj).ok_or_else(|| {
            SeuratError::ParseError(format!(
                "Expected named list or S4 for Assay5, got {:?}",
                robj.sexp_type()
            ))
        })?;
        fields
    ...
    let layers_list = if let RObject::S4Object(s4) = layers_obj {
        ...
        if let Some(data_slot) = s4.attributes.get(".Data") {
            get_named_list_items(data_slot).ok_or_else(|| {
                SeuratError::ParseError("Expected named list in layers .Data".into())
            })?
        } else {
...
fn extract_assay5_main_matrix(robj: &RObject) -> Result<ExpressionMatrix, SeuratError> {
    ...
    if let Some(m) = layers.get("counts") {
        return Ok(m.clone());
    }
    if let Some(m) = layers.get("data") {
        return Ok(m.clone());
    }
    layers
        .into_iter()
...
fn extract_seurat_v3_v4(
    robj: &RObject,
    _file: &RdsFile,
    skipped: &mut SkippedComponents,
) -> Result<SingleCellData, SeuratError> {
    ...
    let active_assay =
        extract_slot_opt(robj, "active.assay")?.and_then(|o| extract_string_scalar(o).ok());
    let embeddings = extract_slot_opt(robj, "reductions")?.and_then(|red| {
        if !should_skip_object(red, "reductions", skipped) {
            extract_reductions(red, n_cells, skipped).ok()
        } else {
            None
        }
    });
    ...
    let spatial =
        extract_slot_opt(robj, "images")?.and_then(|imgs| extract_spatial_data(imgs).ok());
    ...
    for slot in &["graphs", "commands", "tools"] {
        if let Some(obj) = extract_slot_opt(robj, slot)? {
            if let Some(items) = get_named_list_items(obj) {
                for (name, _) in items {
                    skipped.environments.push(format!("{}.{}", slot, name));
                }
            }
        }
    }
...
fn extract_assays_v3_v4(
    ...
    SeuratError,
> {
    let assay_list = get_named_list_items(robj)
        .ok_or_else(|| SeuratError::ParseError("Expected named list for assays".into()))?;
    if assay_list.is_empty() {
        return Err(SeuratError::ParseError("No assays found".into()));
    }
    ...
    let layers = if assay_list.len() > 1 {
        ...
        for (name, assay) in assay_list.iter().skip(1) {
            if let Ok((layer_expr, _)) = extract_single_assay_v3_v4(assay, skipped) {
...
fn extract_single_assay_v3_v4(
    ...
    let expression = if let Some(matrix) = counts {
        if should_skip_object(matrix, "counts", skipped) {
            return Err(SeuratError::ParseError(
                "Expression matrix is not serializable".into(),
            ));
        }
        parse_dgcmatrix(matrix)?
    } else {
        return Err(SeuratError::ParseError(
            "Missing counts/data matrix in Assay".into(),
        ));
    };
...
> {
    ...
    if keep_layers {
        ...
        let has_split = raw_layers.keys().any(|k| {
            ...
            if let Some(p) = prefix {
                raw_layers
                    .keys()
                    .filter(|k2| re.captures(k2).map(|c2| c2[1].to_string()) == Some(p.clone()))
                    .count()
                    > 1
            } else {
                false
    ...
    let layers = if layer_map.is_empty() {
        None
    } else {
        Some(layer_map)
    };
...
fn extract_metadata(robj: &RObject, expected_rows: usize) -> Result<DataFrame, SeuratError> {
    use arrow::array::{
        ArrayRef, BooleanArray, DictionaryArray, Float64Array, Int32Array, StringArray,
    };
    ...
    for (col_name, col_value) in columns {
        ...
        let array: ArrayRef = match col_value {
            RObject::IntegerVector(iv) => {
                if let Some(RObject::StringVector(sv)) = iv.attributes.get("levels") {
                    ...
                    let dict = DictionaryArray::<Int32Type>::try_new(
                        ...
                        Arc::new(StringArray::from(sv.data.clone())),
                    )
                    .map_err(|e| SeuratError::ParseError(e.to_string()))?;
                    Arc::new(dict)
                } else {
                    Arc::new(Int32Array::from(iv.data.clone()))
                }
            }
            ...
            RObject::LogicalVector(lv) => Arc::new(BooleanArray::from(
                lv.data.iter().map(|&v| v != 0).collect::<Vec<_>>(),
            )),
            _ => {
                let placeholder: Vec<String> =
                    (0..expected_rows).map(|i| format!("Value_{}", i)).collect();
                Arc::new(StringArray::from(placeholder))
            }
        };
    ...
    DataFrame::new(col_names, col_arrays, expected_rows)
...
fn extract_reductions(
    ...
    let reduction_list = match get_named_list_items(robj) {
        Some(list) if !list.is_empty() => list,
        _ => return Ok(HashMap::new()),
    };
    let mut embeddings = HashMap::new();
    for (name, reduction) in reduction_list {
        if should_skip_object(reduction, &format!("reductions.{}", name), skipped) {
            continue;
        }
        if let Ok(embedding) = extract_single_reduction(reduction, expected_rows) {
...
fn extract_single_reduction(
    ...
    let s4 = match robj {
        ...
        _ => {
            return Err(SeuratError::ParseError(format!(
                "Expected S4 for DimReduc, got {:?}",
                robj.sexp_type()
            )))
        }
    };
    let cell_embeddings = s4
        .attributes
        .get("cell.embeddings")
        .ok_or_else(|| SeuratError::ParseError("Missing cell.embeddings in DimReduc".into()))?;
    let key = s4
        .attributes
        .get("key")
        .and_then(|o| extract_string_scalar(o).ok())
        .unwrap_or_else(|| "unknown_".to_string());
    ...
    let (data, dim) = match robj {
        RObject::DoubleVector(dv) => {
            if let Some(dim_obj) = dv.attributes.get("dim") {
                (dv.data.clone(), extract_integer_vector(dim_obj)?)
            } else {
...
fn extract_spatial_data(robj: &RObject) -> Result<SpatialData, SeuratError> {
    let image_list = get_named_list_items(robj)
        .ok_or_else(|| SeuratError::ParseError("Expected named list for images".into()))?;
    if image_list.is_empty() {
        return Err(SeuratError::ParseError("No images found".into()));
    }
...
fn extract_single_image(robj: &RObject) -> Result<SpatialData, SeuratError> {
    ...
    let coordinates = s4
        .attributes
        .get("coordinates")
        .ok_or_else(|| SeuratError::ParseError("Missing coordinates in spatial image".into()))?;
    ...
    let scale_factors = s4
        .attributes
        .get("scale.factors")
        .and_then(|sf| extract_scale_factors(sf).ok());
    SpatialData::new(flat_coords, n_dims, None, scale_factors)
        .map_err(|e| SeuratError::ValidationError(e))
}

fn extract_spatial_from_list(robj: &RObject) -> Result<SpatialData, SeuratError> {
    let items = get_named_list_items(robj)
        .ok_or_else(|| SeuratError::ParseError("Expected named list for spatial".into()))?;
    let mut coordinates = None;
    let mut scale_factors = None;
    for (name, value) in items {
        match name {
            "coordinates" => coordinates = Some(value),
            "scale.factors" => scale_factors = Some(value),
            _ => {}
        }
    }
    let coords =
        coordinates.ok_or_else(|| SeuratError::ParseError("Missing coordinates".into()))?;
...
fn extract_scale_factors(robj: &RObject) -> Result<HashMap<String, f64>, SeuratError> {
    let items = get_named_list_items(robj)
        .ok_or_else(|| SeuratError::ParseError("Expected named list for scale.factors".into()))?;
...
mod tests {
    ...
    #[test]
    fn test_seurat_version_display() {
        assert_eq!(format!("{}", SeuratVersion::V5), "V5");
    }
}
