...
use hdf5::{File, Group as H5Group, Dataset as H5Dataset};
...
impl StorageBackend for H5Backend {
    ...
    fn open_rw<P: AsRef<Path>>(path: P) -> Result<Self::Store> {
        let file = File::open_rw(path.as_ref())
            .map_err(|e| CrossCellError::Internal(format!("Failed to open HDF5 file in RW mode: {}", e)))?;
        Ok(H5Store(file))
    }
}
...
impl GroupOp<H5Backend> for H5Store {
    fn list(&self) -> Result<Vec<String>> {
        ...
        for name in self.0.member_names()
    ...
    fn new_group(&self, name: &str) -> Result<H5GroupWrapper> {
        let group = self.0.create_group(name)
            .map_err(|e| CrossCellError::Internal(format!("Failed to create group '{}': {}", name, e)))?;
        Ok(H5GroupWrapper(group))
    }
    
    fn open_group(&self, name: &str) -> Result<H5GroupWrapper> {
        let group = self.0.group(name)
            .map_err(|e| CrossCellError::Internal(format!("Failed to open group '{}': {}", name, e)))?;
        Ok(H5GroupWrapper(group))
    }
    
    fn new_dataset(&self, name: &str, shape: &[usize], dtype: ScalarType) -> Result<H5DatasetWrapper> {
        ...
        let dataset = match dtype {
            ScalarType::I32 => {
                self.0.new_dataset::<i32>()
                    .shape(h5_shape)
                    .create(name)
            },
            ScalarType::I64 => {
                self.0.new_dataset::<i64>()
                    .shape(h5_shape)
                    .create(name)
            },
            ScalarType::F32 => {
                self.0.new_dataset::<f32>()
                    .shape(h5_shape)
                    .create(name)
            },
            ScalarType::F64 => {
                self.0.new_dataset::<f64>()
                    .shape(h5_shape)
                    .create(name)
            },
            ScalarType::Bool => {
                self.0.new_dataset::<bool>()
                    .shape(h5_shape)
                    .create(name)
            },
            _ => return Err(CrossCellError::UnsupportedType { type_name: format!("Unsupported dtype: {:?}", dtype) }),
        }.map_err(|e| CrossCellError::Internal(format!("Failed to create dataset '{}': {}", name, e)))?;
        
        Ok(H5DatasetWrapper(dataset))
    }
    
    fn open_dataset(&self, name: &str) -> Result<H5DatasetWrapper> {
        let dataset = self.0.dataset(name)
            .map_err(|e| CrossCellError::Internal(format!("Failed to open dataset '{}': {}", name, e)))?;
        Ok(H5DatasetWrapper(dataset))
    }
...
impl DatasetOp<H5Backend> for H5DatasetWrapper {
    fn dtype(&self) -> Result<ScalarType> {
        ...
        if dtype.is::<i32>() {
            ...
            Err(CrossCellError::UnsupportedType { type_name: "Unsupported HDF5 dtype".to_string() })
        }
    }
    ...
    fn read_slice(&self, _selection: &[SelectInfo]) -> Result<DynArray> {
        ...
        match dtype {
            ScalarType::I32 => {
                let data: Vec<i32> = self.0.read_raw()
                    .map_err(|e| CrossCellError::Internal(format!("Failed to read data: {}", e)))?;
                Ok(DynArray::I32(data))
            },
            ScalarType::I64 => {
                let data: Vec<i64> = self.0.read_raw()
                    .map_err(|e| CrossCellError::Internal(format!("Failed to read data: {}", e)))?;
                Ok(DynArray::I64(data))
            },
            ScalarType::F32 => {
                let data: Vec<f32> = self.0.read_raw()
                    .map_err(|e| CrossCellError::Internal(format!("Failed to read data: {}", e)))?;
                Ok(DynArray::F32(data))
            },
            ScalarType::F64 => {
                let data: Vec<f64> = self.0.read_raw()
                    .map_err(|e| CrossCellError::Internal(format!("Failed to read data: {}", e)))?;
                Ok(DynArray::F64(data))
            },
            ScalarType::Bool => {
                let data: Vec<bool> = self.0.read_raw()
                    .map_err(|e| CrossCellError::Internal(format!("Failed to read data: {}", e)))?;
    ...
    fn write_slice(&self, data: &DynArray, _selection: &[SelectInfo]) -> Result<()> {
        ...
        match data {
            DynArray::I32(vec) => {
                self.0.write(vec)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to write data: {}", e)))?;
            },
            DynArray::I64(vec) => {
                self.0.write(vec)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to write data: {}", e)))?;
            ...
            _ => return Err(CrossCellError::UnsupportedType { type_name: "Unsupported data type".to_string() }),
        }
        Ok(())
    }
}
...
impl AttributeOp<H5Backend> for H5GroupWrapper {
    fn get_attr(&self, name: &str) -> Result<Value> {
        let attr = self.0.attr(name)
            .map_err(|e| CrossCellError::Internal(format!("Failed to get attribute '{}': {}", name, e)))?;
        ...
        Err(CrossCellError::UnsupportedType { type_name: format!("Unsupported attribute type for '{}'", name) })
    }
    
    fn set_attr(&mut self, name: &str, value: &Value) -> Result<()> {
        match value {
            Value::I32(v) => {
                self.0.new_attr::<i32>().create(name)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to create attribute: {}", e)))?
                    .write_scalar(v)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to write attribute: {}", e)))?;
            },
            Value::I64(v) => {
                self.0.new_attr::<i64>().create(name)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to create attribute: {}", e)))?
                    .write_scalar(v)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to write attribute: {}", e)))?;
            },
            Value::F32(v) => {
                self.0.new_attr::<f32>().create(name)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to create attribute: {}", e)))?
                    .write_scalar(v)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to write attribute: {}", e)))?;
            },
            Value::F64(v) => {
                self.0.new_attr::<f64>().create(name)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to create attribute: {}", e)))?
                    .write_scalar(v)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to write attribute: {}", e)))?;
            },
            Value::Bool(v) => {
                self.0.new_attr::<bool>().create(name)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to create attribute: {}", e)))?
                    .write_scalar(v)
                    .map_err(|e| CrossCellError::Internal(format!("Failed to write attribute: {}", e)))?;
            },
            _ => return Err(CrossCellError::UnsupportedType { type_name: "Unsupported attribute type".to_string()}),
        }
        Ok(())
    }
