...
use crate::rds::{
    RObject, RdsFile, Attributes,
    IntegerVector, LogicalVector, DoubleVector, RawVector, StringVector,
    GenericVector, PairList, S4Object,
    StringEncoding, SymbolIndex,
    Symbol,
};
...
fn simple_attributes_strategy() -> impl Strategy<Value = Attributes> {
    prop::collection::vec(
        (valid_string_strategy(), string_encoding_strategy()),
        0..3
    ).prop_map(|pairs| {
...
fn string_vector_strategy() -> impl Strategy<Value = StringVector> {
    prop::collection::vec(
        (data_string_strategy(), string_encoding_strategy(), any::<bool>()),
        0..20
    ).prop_flat_map(|items| {
        let len = items.len();
        (Just(items), prop::collection::vec(Just(Attributes::default()), 1..=1))
            ...
            // 不使用属性，避免空字符串属性名问题
            StringVector { data, encodings, missing, attributes: Attributes::default() }
        })
    })
}
...
fn simple_robject_strategy() -> impl Strategy<Value = RObject> {
    prop_oneof![
        ...
        prop::collection::vec(any::<i32>(), 0..50).prop_map(|data| {
            RObject::IntegerVector(IntegerVector { data, attributes: Attributes::default() })
        }),
        prop::collection::vec(prop_oneof![Just(0i32), Just(1i32), Just(i32::MIN)], 0..50).prop_map(|data| {
            RObject::LogicalVector(LogicalVector { data, attributes: Attributes::default() })
        }),
        prop::collection::vec(
            prop_oneof![
                any::<f64>().prop_filter("not NaN", |x| !x.is_nan()),
            ...
            RObject::DoubleVector(DoubleVector { data, attributes: Attributes::default() })
        }),
        prop::collection::vec(any::<u8>(), 0..100).prop_map(|data| {
            RObject::RawVector(RawVector { data, attributes: Attributes::default() })
        }),
...
fn generic_vector_strategy() -> impl Strategy<Value = GenericVector> {
    prop::collection::vec(simple_robject_strategy(), 0..10)
        .prop_map(|data| GenericVector { data, attributes: Attributes::default() })
}
...
fn pairlist_strategy() -> impl Strategy<Value = PairList> {
    prop::collection::vec(
        ...
        PairList { data, has_tag, tag_names, tag_encodings, attributes: Attributes::default() }
    })
}
...
fn s4_object_strategy() -> impl Strategy<Value = S4Object> {
    (
        ...
        string_encoding_strategy(),
    ).prop_map(|(class_name, class_encoding, package_name, package_encoding)| {
        S4Object {
            class_name,
            class_encoding,
            package_name,
...
fn test_reference_deduplication() {
    ...
    let pairlist = PairList {
        ...
        tag_encodings: vec![StringEncoding::Utf8, StringEncoding::Utf8, StringEncoding::Utf8],
        attributes: Attributes::default(),
    };
    ...
    if let RObject::PairList(parsed_pl) = &parsed.object {
        ...
        assert_eq!(parsed_pl.has_tag, vec![true, true, true], "All should have tags");
        ...
        // 真正的去重需要在写入时检查已写入的符号。
        assert!(!parsed.symbols.is_empty(), "Should have at least one symbol");
    } else {
...
fn compare_robject(a: &RObject, b: &RObject) -> bool {
    match (a, b) {
        ...
        (RObject::GenericVector(va), RObject::GenericVector(vb)) => {
            ...
            va.data.iter().zip(vb.data.iter()).all(|(x, y)| compare_robject(x, y))
        }
