...
use super::atomic::{parse_double_body, parse_integer_body};
...
use crate::rds::r_object::{DoubleVector, IntegerVector, RObject, StringVector};
    ...
    F: FnMut(&mut R, &mut SharedParseInfo) -> Result<RObject>,
{
    ...
    if sexp_type != SEXPType::List as u8 {
        return Err(RdsError::ParseError {
            ...
            message: format!(
                "Expected ALTREP description to be a pairlist (LIST), got type {}",
                sexp_type
            ),
        });
    }

    let parsed_header = ParsedHeader::from_raw(header).ok_or_else(|| RdsError::ParseError {
        context: "altrep".to_string(),
    ...
    let altrep_type = match &pairlist.data[0] {
        ...
        _ => {
            return Err(RdsError::ParseError {
                context: "altrep".to_string(),
                message: format!(
                    "Expected type specification symbol or string in ALTREP description, got {:?}",
                    pairlist.data[0]
                ),
            });
        }
    ...
    F: FnMut(&mut R, &mut SharedParseInfo) -> Result<RObject>,
{
    ...
    if plist_header[3] != SEXPType::List as u8 {
        return Err(RdsError::ParseError {
            ...
            message: format!(
                "Expected pairlist in wrap_integer payload, got type {}",
                plist_header[3]
            ),
        });
    }
    ...
    let mut int_vec = match &contents {
        RObject::IntegerVector(v) => v.clone(),
        _ => {
            return Err(RdsError::ParseError {
                context: "wrap_integer".to_string(),
                message: format!(
                    "Expected IntegerVector in wrap_integer payload, got {:?}",
                    contents
                ),
            });
        }
    ...
    if meta_header[3] != SEXPType::Int as u8 {
        return Err(RdsError::ParseError {
            context: "wrap_integer".to_string(),
            message: format!(
                "Expected integer vector for metadata, got type {}",
                meta_header[3]
            ),
        });
    }
    ...
    if metadata.data.len() != 2 {
        return Err(RdsError::ParseError {
            context: "wrap_integer".to_string(),
            message: format!(
                "wrap_integer metadata should be length 2, got {}",
                metadata.data.len()
            ),
        });
    }
    ...
    if attr_header[3] == SEXPType::List as u8 {
        let parsed_header =
            ParsedHeader::from_raw(attr_header).ok_or_else(|| RdsError::ParseError {
                context: "wrap_integer".to_string(),
        ...
        for i in 0..wrapper_attrs.names.len() {
            int_vec
                .attributes
                .names
                .push(wrapper_attrs.names[i].clone());
            int_vec
                .attributes
                .encodings
                .push(wrapper_attrs.encodings[i]);
            int_vec
                .attributes
                .values
                .push(wrapper_attrs.values[i].clone());
        }
    } else if attr_header[3] != SEXPType::NilValue as u8 && attr_header[3] != SEXPType::Nil as u8 {
        return Err(RdsError::ParseError {
            context: "wrap_integer".to_string(),
            message: format!(
                "Expected pairlist or NULL for attributes, got type {}",
                attr_header[3]
            ),
        });
    }
    ...
    F: FnMut(&mut R, &mut SharedParseInfo) -> Result<RObject>,
{
    ...
    if plist_header[3] != SEXPType::List as u8 {
        return Err(RdsError::ParseError {
            ...
            message: format!(
                "Expected pairlist in wrap_real payload, got type {}",
                plist_header[3]
            ),
        });
    }
    ...
    let mut double_vec = match &contents {
        RObject::DoubleVector(v) => v.clone(),
        _ => {
            return Err(RdsError::ParseError {
                context: "wrap_real".to_string(),
                message: format!(
                    "Expected DoubleVector in wrap_real payload, got {:?}",
                    contents
                ),
            });
        }
    ...
    if metadata.data.len() != 2 {
        return Err(RdsError::ParseError {
            context: "wrap_real".to_string(),
            message: format!(
                "wrap_real metadata should be length 2, got {}",
                metadata.data.len()
            ),
        });
    }
...
fn parse_compact_intseq<R: Read>(reader: &mut R) -> Result<RObject> {
    ...
    if info.data.len() != 3 {
        return Err(RdsError::ParseError {
            context: "compact_intseq".to_string(),
            message: format!(
                "Expected 3 elements for sequence info, got {}",
                info.data.len()
            ),
        });
    }
    ...
    F: FnMut(&mut R, &mut SharedParseInfo) -> Result<RObject>,
{
    ...
    if plist_header[3] != SEXPType::List as u8 {
        return Err(RdsError::ParseError {
            ...
            message: format!(
                "Expected pairlist in deferred_string payload, got type {}",
                plist_header[3]
            ),
        });
    }
    ...
    let output = match contents {
        ...
        _ => {
            return Err(RdsError::ParseError {
                context: "deferred_string".to_string(),
                message: format!(
                    "Unsupported content type in deferred_string: {:?}",
                    contents
                ),
            });
        }
