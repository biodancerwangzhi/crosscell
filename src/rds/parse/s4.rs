...
use crate::rds::r_object::{S4Object, RObject};
...
pub fn parse_s4_body<R: Read, F>(
    reader: &mut R, _header: &ParsedHeader, shared: &mut SharedParseInfo, parse_fn: &mut F,
) -> Result<S4Object>
                ...
                if let Some(pkg_idx) = sv.attributes.names.iter().position(|n| n == "package") {
                    if let Some(RObject::StringVector(pkg_sv)) = sv.attributes.values.get(pkg_idx) {
                        if !pkg_sv.data.is_empty() {
                            ...
                            s4.package_encoding = pkg_sv.encodings.get(0).copied().unwrap_or_default();
                        }
                    }
                }
            }
        } else {
            s4.attributes.add(tag_name.clone(), val.clone(), tag_encoding);
        }
    }
