...
use super::header::{ParsedHeader, has_tag, encoding_flags};
...
fn parse_language_tag<R: Read>(
    ...
    if sexp_type == SEXPType::Ref as u8 {
        ...
        Err(RdsError::ParseError {
            context: "language tag".into(),
            message: format!("Expected CHAR (9), Ref (255), or Sym (1), got {}", sexp_type),
        })
    }
}
...
where F: FnMut(&mut R, &mut SharedParseInfo) -> Result<RObject> {
    ...
    loop {
        if has_tag(&cur.raw) {
            ...
            if next.sexp_type == SEXPType::NilValue { break; }
...
mod tests {
    ...
    fn list_hdr(tag: bool) -> [u8; 4] { [0, 0, if tag { flags::HAS_TAG } else { 0 }, 2] }
    fn nil_hdr() -> [u8; 4] { [0, 0, 0, SEXPType::NilValue as u8] }
    ...
    fn test_simple_language() {
        ...
        let mut pf = |_: &mut Cursor<Vec<u8>>, _: &mut SharedParseInfo| -> Result<RObject> { Ok(RObject::Null) };
