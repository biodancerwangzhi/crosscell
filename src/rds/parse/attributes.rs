//! 属性解析
use super::header::{has_tag, ParsedHeader};
...
pub fn parse_attributes<R: Read, F>(
    r: &mut R,
    s: &mut SharedParseInfo,
    f: &mut F,
) -> Result<Attributes>
where
    F: FnMut(&mut R, &mut SharedParseInfo) -> Result<RObject>,
{
    ...
    if p.sexp_type != SEXPType::List {
        return Err(RdsError::ParseError {
            context: "attr".into(),
            message: format!("Expected LIST, got {:?}", p.sexp_type),
        });
    }
    parse_attributes_body(r, &p, s, f)
}

pub fn parse_attributes_body<R: Read, F>(
    r: &mut R,
    h: &ParsedHeader,
    s: &mut SharedParseInfo,
    f: &mut F,
