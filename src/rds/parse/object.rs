...
/// # 错误
/// 如果遇到不支持的类型或解析失败返回错误
pub fn parse_object<R: Read>(reader: &mut R, shared: &mut SharedParseInfo) -> Result<RObject> {
...
pub fn parse_object_with_header<R: Read>(
    ...
    // 创建递归解析闭包
    let mut parse_fn =
        |r: &mut R, s: &mut SharedParseInfo| -> Result<RObject> { parse_object(r, s) };
    ...
    let obj = match header.sexp_type {
        ...
        SEXPType::GlobalEnv => RObject::EnvironmentIndex(parse_global_environment_body()),

        SEXPType::BaseEnv => RObject::EnvironmentIndex(parse_base_environment_body()),

        SEXPType::EmptyEnv => RObject::EnvironmentIndex(parse_empty_environment_body()),
        ...
        // ALTREP
        // ========================================
        SEXPType::Altrep => parse_altrep_body(reader, shared, &mut parse_fn)?,
        ...
        SEXPType::Weakref => {
            ...
            RObject::ExternalPointerIndex(crate::rds::r_object::ExternalPointerIndex {
                index: ptr_idx,
            })
        }
    ...
    F: FnMut(&mut R, &mut SharedParseInfo) -> Result<RObject>,
{
    ...
    for _i in 0..n {
        ...
        match const_type {
            ...
            | Some(SEXPType::AttrLang)
            | Some(SEXPType::AttrList) => {
                // 解析字节码语言对象
    ...
    F: FnMut(&mut R, &mut SharedParseInfo) -> Result<RObject>,
{
    ...
    match sexp_type {
        ...
        Some(SEXPType::BcRepDef) => {
            ...
            // 确定是否有属性
            let has_attr =
                actual_type == SEXPType::AttrLang as u8 || actual_type == SEXPType::AttrList as u8;
        ...
        | Some(SEXPType::AttrList) => {
            let has_attr =
                sexp_type == Some(SEXPType::AttrLang) || sexp_type == Some(SEXPType::AttrList);
