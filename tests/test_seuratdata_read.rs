...
fn test_seurat_v4_pbmc3k_read() {
    ...
    assert_eq!(
        ...
        "Expected 2700 cells, got {}",
        result.data.metadata.n_cells
    );

    // Verify gene count
    assert_eq!(
        result.data.metadata.n_genes, 13714,
        "Expected 13714 genes, got {}",
        result.data.metadata.n_genes
    );
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "Expected CLOSXP/unsupported type error, got: {}",
                err_msg
            );
...
fn test_seurat_v4_metadata_columns() {
    ...
    assert!(
        !col_names.is_empty(),
        "Expected at least some metadata columns"
    );
...
fn test_expression_matrix_sparsity() {
    ...
    assert!(
        sparsity > 0.85,
        "Expected sparsity > 85%, got {:.2}%",
        sparsity * 100.0
    );
    assert!(
        sparsity < 0.999,
        "Sparsity too high ({:.2}%), data may be corrupted",
        sparsity * 100.0
    );
