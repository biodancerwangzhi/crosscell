...
fn test_pbmc3k_h5ad_roundtrip() {
    ...
    println!("Original: {} cells x {} genes", 
...
fn test_pbmc3k_cross_format() {
    ...
    println!("From RDS: {} cells x {} genes", 
    ...
    // 跨格式转换可能有一些精度损失，放宽要求
    assert!(report.expression_accuracy.correlation > 0.999, 
        "Cross-format correlation too low: {}", report.expression_accuracy.correlation);
}
...
fn test_pancreas_cross_format() {
    ...
    println!("From RDS: {} cells x {} genes", 
        from_rds.metadata.n_cells, from_rds.metadata.n_genes);
    println!("Cell metadata columns after RDS: {}", from_rds.cell_metadata.n_cols());
    ...
    if let Some(ref meta_acc) = report.cell_metadata_accuracy {
        assert!(meta_acc.categorical_match, 
...
fn test_visium_h5ad_roundtrip() {
    ...
    // 读取原始文件
    let original = read_h5ad(original_path).expect("Failed to read original");
    println!("Original: {} spots x {} genes", 
        original.metadata.n_cells, original.metadata.n_genes);
    ...
    if let Some(spatial_acc) = report.embedding_accuracies.get("spatial") {
        assert!(spatial_acc.max_error < 0.01, 
            "Spatial coordinates error too large: {}", spatial_acc.max_error);
    }
}
...
fn test_visium_cross_format() {
    ...
    if let Some(ref emb) = original.embeddings {
        ...
        if let Some(spatial) = emb.get("spatial") {
            println!("  spatial: {} spots x {} dims", spatial.n_rows, spatial.n_cols);
        }
    }
    ...
    // 3. 读取 RDS
    let from_rds = seurat_rds_to_ir(rds_path).expect("Failed to read rds");
    println!("From RDS: {} spots x {} genes", 
        from_rds.metadata.n_cells, from_rds.metadata.n_genes);
    ...
    if let Some(spatial_acc) = report.embedding_accuracies.get("spatial") {
        assert!(spatial_acc.max_error < 0.01, 
            "Spatial coordinates error too large: {}", spatial_acc.max_error);
        println!("Spatial coordinates preserved: max_error = {:.2e}", spatial_acc.max_error);
    }
}
...
fn test_medium_sparse_roundtrip() {
    ...
    let original = read_h5ad(original_path).expect("Failed to read");
    println!("Original: {} cells x {} genes, NNZ: {}", 
    ...
    // 检查稀疏性保留
    assert!(report.expression_accuracy.sparsity_preserved > 0.999,
        "Sparsity not preserved: {}", report.expression_accuracy.sparsity_preserved);
}
...
fn test_seurat_minimal_roundtrip() {
    ...
    // RDS 往返可能有一些差异，放宽要求
    assert!(report.expression_accuracy.correlation > 0.99,
        "RDS roundtrip correlation too low: {}", report.expression_accuracy.correlation);
}
...
fn test_single_column_roundtrip() {
    ...
    // 验证是单列矩阵
    assert_eq!(original.metadata.n_genes, 1, "Expected single column matrix");
...
fn test_empty_matrix_handling() {
    ...
    match read_h5ad(original_path) {
        Ok(data) => {
            println!("Empty matrix read successfully: {} x {}", 
            ...
            if let Ok(()) = write_h5ad(&data, temp_path) {
                if let Ok(roundtrip) = read_h5ad(temp_path) {
                    println!("Roundtrip successful: {} x {}", 
...
fn test_single_row_cross_format() {
    ...
    // 验证
    assert!(report.expression_accuracy.correlation > 0.999, 
        "Single row cross-format correlation too low: {}", report.expression_accuracy.correlation);
    assert_eq!(roundtrip.metadata.n_cells, 1, "Cell count changed in cross-format");
}
...
fn test_single_column_cross_format() {
    ...
    // 验证
    assert!(report.expression_accuracy.correlation > 0.999, 
        "Single column cross-format correlation too low: {}", report.expression_accuracy.correlation);
    assert_eq!(roundtrip.metadata.n_genes, 1, "Gene count changed in cross-format");
}
...
fn test_sparsity_preservation() {
    ...
    if check_test_data(small_path) {
        ...
        println!("Small sparse: {} x {}, NNZ={}, sparsity={:.2}%", 
            original.metadata.n_cells, original.metadata.n_genes,
            original_nnz, original_sparsity * 100.0);
        ...
        assert_eq!(original_nnz, roundtrip_nnz, 
            "NNZ changed: {} -> {}", original_nnz, roundtrip_nnz);
    ...
    if check_test_data(medium_path) {
        ...
        println!("Medium sparse: {} x {}, NNZ={}, sparsity={:.2}%", 
            original.metadata.n_cells, original.metadata.n_genes,
            original_nnz, original_sparsity * 100.0);
...
fn test_pbmc3k_gold_standard() {
    ...
    let crosscell_output = seurat_rds_to_ir(crosscell_rds_path).expect("Failed to read CrossCell RDS");
    println!("CrossCell RDS: {} cells x {} genes", 
        crosscell_output.metadata.n_cells, crosscell_output.metadata.n_genes);
    
    // 5. 比较 CrossCell 输出与金标准
    let report = calculate_full_accuracy(&gold_standard, &crosscell_output, "PBMC3k vs Gold Standard");
    print_report(&report);
...
fn test_pancreas_gold_standard() {
    ...
    // 5. 比较 CrossCell 输出与金标准
    let report = calculate_full_accuracy(&gold_standard, &crosscell_output, "Pancreas vs Gold Standard");
    print_report(&report);
...
fn test_visium_gold_standard() {
    ...
    // 1. 读取原始 H5AD
    let original = read_h5ad(h5ad_path).expect("Failed to read h5ad");
    println!("Original H5AD: {} spots x {} genes", 
        original.metadata.n_cells, original.metadata.n_genes);
    
    // 2. 读取金标准 RDS（由 CrossCell 生成）
    let gold_standard = seurat_rds_to_ir(gold_rds_path).expect("Failed to read gold standard");
    println!("Gold Standard RDS: {} spots x {} genes", 
        gold_standard.metadata.n_cells, gold_standard.metadata.n_genes);
    ...
    // 4. 读取 CrossCell 生成的 RDS
    let crosscell_output = seurat_rds_to_ir(crosscell_rds_path).expect("Failed to read CrossCell RDS");
    println!("CrossCell RDS: {} spots x {} genes", 
        crosscell_output.metadata.n_cells, crosscell_output.metadata.n_genes);
    
    // 5. 比较 CrossCell 输出与金标准
    let report = calculate_full_accuracy(&gold_standard, &crosscell_output, "Visium vs Gold Standard");
    print_report(&report);
    ...
    assert!(report.expression_accuracy.correlation > 0.9999, 
...
fn test_gold_standard_summary() {
    ...
    let datasets = vec![
        ("PBMC 3k", "tests/data/real_datasets/pbmc3k.h5ad", 
         "tests/data/real_datasets/pbmc3k_easyscf.rds", true),
        ("Pancreas", "tests/data/real_datasets/pancreas.h5ad", 
         "tests/data/real_datasets/pancreas_easyscf.rds", true),
        ("Visium", "tests/data/real_datasets/visium_heart.h5ad", 
         "tests/data/real_datasets/visium_easyscf.rds", false), // CrossCell fallback
    ];
            ...
            (Ok(original), Ok(gold_standard)) => {
                ...
                if write_seurat_rds(&original, &temp_rds).is_ok() {
                    if let Ok(crosscell_output) = seurat_rds_to_ir(&temp_rds) {
                        let report = calculate_full_accuracy(&gold_standard, &crosscell_output, name);
                        ...
                        results.push((name.to_string(), report.expression_accuracy.correlation > 0.99));
                    }
