...
use crosscell::rds::{IntegerVector, StringVector, StringEncoding, GenericVector};
...
fn test_write_simple_named_list() {
    ...
    assert!(result.is_ok(), "Failed to write named list: {:?}", result.err());
...
fn test_write_nested_named_list() {
    let inner = make_named_list(vec![
        ("x", RObject::IntegerVector(IntegerVector { data: vec![1, 2], ..Default::default() })),
        ("y", RObject::IntegerVector(IntegerVector { data: vec![3, 4], ..Default::default() })),
    ]);
    let outer = make_named_list(vec![
        ("data", inner),
        ("name", make_string_vec(&["test"])),
    ]);

    let path = "tests/data/test_nested_named_list_write.rds";
    let result = write_rds(Path::new(path), &outer);
    assert!(result.is_ok(), "Failed to write nested named list: {:?}", result.err());

    let read_result = read_rds(Path::new(path));
    assert!(read_result.is_ok(), "Failed to read back: {:?}", read_result.err());
}
...
fn test_write_named_list_with_class_attribute() {
    ...
    gv.data.push(RObject::IntegerVector(IntegerVector { data: vec![1, 2, 3], ..Default::default() }));
    ...
    gv.attributes.add("class".to_string(), RObject::StringVector(class_sv), StringEncoding::Utf8);
...
fn test_write_simple_integer() {
    let int_vec = RObject::IntegerVector(IntegerVector { data: vec![1, 2, 3, 4, 5], ..Default::default() });

    let path = "tests/data/test_simple_int.rds";
    let result = write_rds(Path::new(path), &int_vec);
    assert!(result.is_ok(), "Failed to write integer vector: {:?}", result.err());
