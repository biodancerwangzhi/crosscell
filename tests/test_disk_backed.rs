...
use crosscell::ir::expression::{SparseFormat, SparseMatrixCSR, SparseMatrixCSC, ExpressionMatrix};
...
fn test_chunk_store_create_and_open() {
    ...
    let store = ChunkStore::create(
        &path, 1000, 500, 100, SparseFormat::CSR, true
    ).unwrap();
...
fn test_chunk_store_csr_write_read() {
    ...
    let mut store = ChunkStore::create(
        &path, 100, 50, 25, SparseFormat::CSR, true
    ).unwrap();
...
fn test_chunk_store_csc_write_read() {
    ...
    // 创建存储
    let mut store = ChunkStore::create(
        &path, 100, 50, 25, SparseFormat::CSC, true
    ).unwrap();
...
fn test_chunk_store_compression() {
    ...
    let mut store_c = ChunkStore::create(
        &path_compressed, n_rows, n_cols, n_rows, SparseFormat::CSR, true
    ).unwrap();
    store_c.write_csr_chunk(0, &matrix).unwrap();
    
    // 写入未压缩版本
    let mut store_u = ChunkStore::create(
        &path_uncompressed, n_rows, n_cols, n_rows, SparseFormat::CSR, false
    ).unwrap();
...
fn test_disk_backed_matrix_from_csr() {
    ...
    let matrix = SparseMatrixCSR::new(data.clone(), indices.clone(), indptr.clone(), n_rows, n_cols).unwrap();
...
fn test_disk_backed_matrix_process_chunks() {
    ...
    // 流式处理每个块
    let results = disk_matrix.process_chunks(|idx, chunk| {
