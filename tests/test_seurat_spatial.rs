...
fn test_seurat_spatial_extraction() {
    ...
    assert_eq!(spatial.coordinates.len(), 200, "Should have 200 coordinate values (100 cells × 2 dims)");
    
    // Verify scale factors
    assert!(spatial.scale_factors.is_some(), "Scale factors should be present");
...
fn test_seurat_spatial_coordinates() {
    ...
    assert!(coord_0.is_some(), "Should be able to get coordinates for cell 0");
    assert_eq!(coord_0.unwrap().len(), 2, "Coordinates should be 2D");
    
    let coord_99 = spatial.get_coordinates(99);
    assert!(coord_99.is_some(), "Should be able to get coordinates for cell 99");
    
    let coord_100 = spatial.get_coordinates(100);
    assert!(coord_100.is_none(), "Should return None for out-of-bounds index");
...
fn test_seurat_spatial_roundtrip() {
    ...
    let result = seurat_rds_to_ir("tests/data/seurat_spatial_simplified.rds");
    assert!(result.is_ok(), "Failed to read Seurat spatial: {:?}", result.err());
    ...
    assert!(write_result.is_ok(), "Failed to write Seurat spatial: {:?}", write_result.err());
    ...
    assert!(roundtrip_result.is_ok(), "Failed to read roundtrip Seurat: {:?}", roundtrip_result.err());

    let roundtrip_ir = roundtrip_result.unwrap();

    // 6. Verify spatial data was preserved
    assert!(roundtrip_ir.spatial.is_some(), "Spatial data not preserved in roundtrip");
    ...
    for i in [0, 49, 99].iter() {
        ...
        for j in 0..spatial.n_dims {
            ...
            assert!(diff < 1e-7, "Coordinate mismatch at cell {}, dim {}: {} vs {}", 
                    i, j, orig_coords[j], roundtrip_coords[j]);
        }
    }

    // Check scale factors
    if let Some(orig_sf) = &spatial.scale_factors {
        assert!(roundtrip_spatial.scale_factors.is_some(), "Scale factors not preserved");
        let roundtrip_sf = roundtrip_spatial.scale_factors.as_ref().unwrap();
        
        for (key, value) in orig_sf {
            assert!(roundtrip_sf.contains_key(key), "Scale factor '{}' missing", key);
            let roundtrip_value = roundtrip_sf.get(key).unwrap();
            let diff = (value - roundtrip_value).abs();
            assert!(diff < 1e-7, "Scale factor '{}' mismatch: {} vs {}", key, value, roundtrip_value);
        }
    }
