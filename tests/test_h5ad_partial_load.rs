...
fn test_inspect_h5ad_sparse() {
    ...
    assert!(result.is_ok(), "Failed to inspect small_sparse.h5ad: {:?}", result.err());
    ...
    // 验证元数据列数
    assert!(info.n_obs_columns > 0, "Expected some cell metadata columns");
    assert!(info.n_var_columns > 0, "Expected some gene metadata columns");
...
fn test_inspect_h5ad_dense() {
    ...
    assert!(result.is_ok(), "Failed to inspect small_dense.h5ad: {:?}", result.err());
...
fn test_partial_load_metadata_only() {
    ...
    assert!(result.is_ok(), "Failed to read with metadata_only: {:?}", result.err());
    ...
    assert!(data.cell_metadata.n_cols() > 0, "Expected cell metadata to be loaded");
    
    assert_eq!(data.gene_metadata.n_rows, 50);
    assert!(data.gene_metadata.n_cols() > 0, "Expected gene metadata to be loaded");
    
    // 验证嵌入未加载
    assert!(data.embeddings.is_none(), "Expected embeddings to not be loaded");
...
fn test_partial_load_lazy() {
    ...
    let result = read_h5ad_partial("tests/data/small_sparse.h5ad", &options);
    assert!(result.is_ok(), "Failed to read with lazy mode: {:?}", result.err());
    ...
    match &data.expression {
        ExpressionMatrix::Lazy(lazy) => {
            ...
            println!("  ✓ Expression matrix is lazy with nnz={}", lazy.nnz.unwrap());
        }
    ...
    // 验证嵌入已加载
    assert!(data.embeddings.is_some(), "Expected embeddings to be loaded");
...
fn test_partial_load_full() {
    ...
    let result = read_h5ad_partial("tests/data/small_sparse.h5ad", &options);
    assert!(result.is_ok(), "Failed to read with full mode: {:?}", result.err());
    ...
    match &data.expression {
        ExpressionMatrix::SparseCSR(csr) => {
            ...
            println!("  ✓ Expression matrix fully loaded: {} × {}", csr.n_rows, csr.n_cols);
        }
...
fn test_partial_load_custom_options() {
    ...
    let result = read_h5ad_partial("tests/data/small_sparse.h5ad", &options);
    assert!(result.is_ok(), "Failed to read with custom options: {:?}", result.err());
    ...
    // 验证 pairwise 未加载
    assert!(data.cell_pairwise.is_none(), "Expected cell_pairwise to not be loaded");
    assert!(data.gene_pairwise.is_none(), "Expected gene_pairwise to not be loaded");
...
fn test_h5ad_info_estimate_memory() {
    ...
    println!("  Estimated full memory: {:.2} MB", estimated_memory as f64 / 1024.0 / 1024.0);
...
fn test_inspect_real_dataset() {
    ...
    assert!(result.is_ok(), "Failed to inspect pbmc3k: {:?}", result.err());
    ...
    println!("✓ Successfully inspected real dataset: {} cells × {} genes", 
