...
use crosscell::seurat::{ir_to_seurat::write_seurat_rds, seurat_rds_to_ir};
use std::path::Path;
...
fn test_anndata_spatial_coordinates_extraction() {
    ...
    assert!(
        data.spatial.is_some(),
        "Spatial coordinates should be extracted"
    );
...
fn test_anndata_scale_factors_extraction() {
    ...
    assert!(
        ...
        "Scale factors should be extracted"
    );
...
fn test_anndata_spatial_roundtrip_coordinates() {
    ...
    for (i, (&orig, &rt)) in orig_spatial
        .coordinates
        .iter()
        .zip(rt_spatial.coordinates.iter())
        .enumerate()
    {
        let error = (orig - rt).abs();
        assert!(
            error < 0.01,
            "Coordinate {} error {} exceeds 0.01 pixel threshold",
            i,
            error
        );
    }
...
fn test_anndata_spatial_roundtrip_images() {
    ...
    for orig_img in orig_images {
        ...
        for (i, (&orig_pixel, &rt_pixel)) in
            orig_img.data.iter().zip(rt_img.data.iter()).enumerate()
        {
            assert_eq!(
                orig_pixel, rt_pixel,
...
fn test_anndata_spatial_roundtrip_scale_factors() {
    ...
    for (name, &orig_value) in orig_sf {
        ...
        assert!(
            error < 1e-6,
            "Scale factor '{}' error {} exceeds 1e-6 threshold",
            name,
            error
        );
    }
...
fn test_cross_format_anndata_to_seurat_coordinates() {
    ...
    for i in 0..anndata_spatial.n_cells() {
        ...
        for j in 0..anndata_spatial.n_dims {
            ...
            assert!(
                error < 0.01,
                "Cross-format coordinate error at cell {}, dim {}: {}",
                i,
                j,
                error
            );
...
fn test_cross_format_anndata_to_seurat_scale_factors() {
    ...
    for (name, &anndata_value) in anndata_sf {
        ...
        assert!(
            error < 1e-6,
            "Cross-format scale factor '{}' error: {}",
            name,
            error
        );
    }
...
fn test_coordinate_system_alignment() {
    ...
    for i in 0..orig_coords.len() {
        ...
        assert!(
            orig_to_seurat_error < 0.01,
            "Alignment error (AnnData→Seurat) at index {}: {}",
            i,
            orig_to_seurat_error
        );
        assert!(
            orig_to_rt_error < 0.01,
            "Alignment error (full cycle) at index {}: {}",
            i,
            orig_to_rt_error
        );
    }
...
fn test_non_spatial_data() {
    ...
    if result.is_ok() {
        ...
        assert!(
            ...
            "Non-spatial data should not have spatial field"
        );
    }
