...
use crosscell::rds::{IntegerVector, StringVector, StringEncoding};
...
fn test_simple_with_attributes() {
    ...
    let mut iv = IntegerVector { data: vec![1, 2, 3], ..Default::default() };
    let mut class_sv = StringVector::default();
    class_sv.add("myclass".to_string(), StringEncoding::Utf8);
    iv.attributes.add("class".to_string(), RObject::StringVector(class_sv), StringEncoding::Utf8);
