...
fn test_read_spatial_with_images() {
    ...
    assert_eq!(hires_img.data.len(), 200 * 200 * 3, "Hires should have 200×200×3 bytes");
    ...
    assert_eq!(lowres_img.data.len(), 100 * 100 * 3, "Lowres should have 100×100×3 bytes");
...
fn test_read_spatial_3d() {
    ...
    assert_eq!(spatial.coordinates.len(), 150, "Should have 150 coordinate values (50 cells × 3 dims)");
...
fn test_read_spatial_no_images() {
    ...
    assert!(result.is_ok(), "Failed to read spatial test file (no images)");
...
fn test_read_non_spatial_h5ad() {
    ...
    if result.is_ok() {
        let data = result.unwrap();
        // 验证没有空间数据
        assert!(data.spatial.is_none(), "Non-spatial file should not have spatial data");
    }
}
...
fn test_spatial_coordinate_access() {
    ...
    assert!(invalid_coords.is_none(), "Out of bounds access should return None");
}
