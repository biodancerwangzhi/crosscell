...
fn test_h5ad_roundtrip_with_validation_function() {
    ...
    match result {
        Ok(report) => {
            ...
            assert!(report.passed(), "Validation failed: {}", report.detailed_report());
        }
...
fn test_h5ad_metadata_preservation() {
    ...
    // 验证元数据
    assert_eq!(
        original.cell_metadata.n_rows,
        roundtrip.cell_metadata.n_rows,
    ...
    assert_eq!(
        original.gene_metadata.n_rows,
        roundtrip.gene_metadata.n_rows,
...
fn test_h5ad_embeddings_preservation() {
    ...
    // 验证嵌入
    assert!(roundtrip.embeddings.is_some(), "Embeddings lost during roundtrip");
