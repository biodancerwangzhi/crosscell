...
fn test_spatial_roundtrip_with_images() {
    ...
    // 验证空间数据仍然存在
    assert!(
        roundtrip_data.spatial.is_some(),
        "Spatial data should be preserved"
    );
    ...
    assert_eq!(
        rt_spatial.n_dims, orig_n_dims,
        "Dimension count should match"
    );

    // 验证坐标数据
    assert_eq!(
        rt_spatial.coordinates.len(),
        orig_coords.len(),
        "Coordinate array length should match"
    );
    for (i, (&orig, &rt)) in orig_coords
        .iter()
    ...
    if let (Some(orig_images), Some(rt_images)) = (&spatial.images, &rt_spatial.images) {
        ...
        for orig_img in orig_images {
            ...
            // 验证图像属性
            assert_eq!(
                rt_img.width, orig_img.width,
                "Image '{}' width should match",
                orig_img.name
            );
            assert_eq!(
                rt_img.height, orig_img.height,
                "Image '{}' height should match",
                orig_img.name
            );
            assert_eq!(
                rt_img.data.len(),
                orig_img.data.len(),
                "Image '{}' data length should match",
                orig_img.name
            );
    ...
    if let (Some(orig_sf), Some(rt_sf)) = (&spatial.scale_factors, &rt_spatial.scale_factors) {
        assert_eq!(
            orig_sf.len(),
            rt_sf.len(),
            "Number of scale factors should match"
        );
...
fn test_spatial_roundtrip_no_images() {
    ...
    assert!(
        rt_spatial.scale_factors.is_none(),
        "Should still not have scale factors"
    );
