...
fn test_seurat_v5_small_expression_matrix() {
    ...
    match &ir.expression {
        crosscell::ir::ExpressionMatrix::SparseCSC(csc) => {
            ...
            // 验证稀疏矩阵结构
            assert_eq!(csc.indptr.len(), csc.n_cols + 1, "indptr 长度应为 n_cols + 1");
            assert_eq!(csc.data.len(), csc.indices.len(), "data 和 indices 长度应相等");
        }
    ...
    match result {
        Ok(ir) => {
            ...
            if let Some(ref embeddings) = ir.embeddings {
                ...
                // 应该有 PCA 和 UMAP
                assert!(embeddings.contains_key("pca") || embeddings.contains_key("PCA"), 
                    "应包含 PCA 降维");
                assert!(embeddings.contains_key("umap") || embeddings.contains_key("UMAP"), 
                    "应包含 UMAP 降维");
            } else {
                println!("  ⚠️ 未检测到降维结果");
            }
...
fn test_seurat_v5_medium_embeddings() {
    ...
    if let Some(ref embeddings) = ir.embeddings {
        // 检查 PCA
        let pca_key = if embeddings.contains_key("pca") { "pca" } else { "PCA" };
        ...
        // 检查 UMAP
        let umap_key = if embeddings.contains_key("umap") { "umap" } else { "UMAP" };
